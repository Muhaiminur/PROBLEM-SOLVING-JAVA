...
public class cse111Lab10task4a{
  public static void main(String[]args){
    ...
    int sum=0;int c=0;
    while(c<x.length()){
      sum=sum+(int)Math.pow(Integer.parseInt(Character.toString(x.charAt(c))),x.length());
      c++;
    }
    if(sum==Integer.parseInt(x)){
    System.out.println(x+"== is a Narcissistic Number");
    }else{
        System.out.println(x+"== is not a Narcissistic Number");
    }
  }
}